  ...
  implements vscode.TreeDataProvider<Entry>, vscode.FileSystemProvider
{
  ...
  async add(selected?: Entry) {
    ...
    if (selected) {
      const filePath =
        selected.type === vscode.FileType.Directory
      ...
      this._writeFile(
        filePath,
        this.stringToUnit8Array(JSON.stringify(command)),
        { create: true, overwrite: true },
      );
    } else {
      this._writeFile(
        `${this.rootUri.fsPath}/${fileName}.json`,
        this.stringToUnit8Array(JSON.stringify(command)),
        { create: true, overwrite: true },
      );
    }
  ...
  addFolder(selected?: Entry) {
    vscode.window
      .showInputBox({ placeHolder: "Enter a new group name" })
      .then((value) => {
        if (value !== null && value !== undefined) {
          if (selected) {
            const filePath =
              selected.type === vscode.FileType.Directory
                ? `${selected.uri.fsPath}/${value}`
                : `${this.getDirectoryPath(selected.uri.fsPath)}/${value}`;
            this.createDirectory(vscode.Uri.file(filePath));
          } else {
            this.createDirectory(
              vscode.Uri.file(`${this.rootUri.fsPath}/${value}`),
            );
          }
        }
      });
    ...
    if (element && element.type === vscode.FileType.File) {
      ...
      vscode.window
        .showInputBox({
          ...
          value: this.getFileName(element.uri.fsPath),
        })
        .then((value) => {
          if (value !== null && value !== undefined) {
            const newPath = vscode.Uri.file(
              `${this.getDirectoryPath(element.uri.fsPath)}/${value}`,
            );
  ...
  async getChildren(element?: Entry): Promise<Entry[]> {
    ...
    return children
      .filter(
        ([name, type]) =>
          this.isJson(name) || type === vscode.FileType.Directory,
      )
      .map(([name, type]) => ({
  ...
  getDirectoryPath(directoryPath: string): string {
    return directoryPath.slice(0, directoryPath.lastIndexOf(path.sep));
  }
...
export class CommandExplorer {
  ...
  constructor(viewId: string, storagePath: string) {
    this.setupStorage(storagePath).then(() => {
      ...
      vscode.commands.registerCommand(`${viewId}.openFile`, () =>
        this.openResource(
          "Open the JSON files where the commands are stored in a new window?",
          vscode.Uri.file(storagePath),
        ),
      );
      this.commandExplorer.onDidChangeSelection(
        (event) => (this.selectedFile = event.selection[0]),
      );
      vscode.commands.registerCommand(`${viewId}.add`, () =>
        treeDataProvider.add(this.selectedFile),
      );
      vscode.commands.registerCommand(`${viewId}.addFolder`, () =>
        treeDataProvider.addFolder(this.selectedFile),
      );
      vscode.commands.registerCommand(`${viewId}.sync`, () =>
        treeDataProvider.refresh(),
      );
      vscode.commands.registerCommand(`${viewId}.edit`, (element) =>
        treeDataProvider.edit(element),
      );
      vscode.commands.registerCommand(`${viewId}.delete`, (element: Entry) =>
        treeDataProvider.delete(element.uri, { recursive: true }),
      );
      vscode.commands.registerCommand(`${viewId}.goToFile`, (element: Entry) =>
        treeDataProvider.delete(element.uri, { recursive: true }),
      );
    });
  }
